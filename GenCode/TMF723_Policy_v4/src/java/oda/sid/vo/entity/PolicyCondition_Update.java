...
public class PolicyCondition_Update extends AbstractEntity implements java.io.Serializable {
    private final static long serialVersionUID = 1L;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyCondition_Update_Note")
    private List<Note> note;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyCondition_Update_PolicyConditionStatement")
    private List<PolicyConditionStatement> policyConditionStatement;
    @Embedded
    private TimePeriod validFor;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyCondition_Update_PolicyCondition")
