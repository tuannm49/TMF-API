...
public class PolicyAction_Update extends AbstractEntity implements java.io.Serializable {
    private final static long serialVersionUID = 1L;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyAction_Update_Note")
    private List<Note> note;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyAction_Update_PolicyAction")
    private List<PolicyAction> policyAction;
    @OneToMany(cascade = CascadeType.PERSIST)
    @JoinTable(name = "PolicyAction_Update_PolicyConditionRef")
