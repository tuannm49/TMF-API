...
import oda.api.tmf.commons.base.annotation.EntityReferenceProperty;
...
public class ReferencedEntityGetter<T> {
    ...
    private HashMap<String, Class> createReferenceFields_() {
        ...
        for (String externalFieldName : classFields.getExternalFieldNames()) {
            ...
            if (AbstractEntityRef.class.isAssignableFrom(fieldDataClass) == false) {
                continue;
            }
    ...
    private void getOneReferencedEntity_(T entity, Class entityClass, int depth, String internalFieldName) {
        ...
        if ((fieldValue instanceof AbstractEntityRef) == true) {
            getReferencedEntity_((AbstractEntityRef) fieldValue, entityClass, depth);
            return;
        }
    ...
    private void getArrayReferencedEntity_(Object fieldValue, Class entityClass, int depth) {
        AbstractEntityRef entityReferences [];
        try {
            entityReferences = (AbstractEntityRef[]) fieldValue;
        }
        catch (Exception ex) {
    ...
    private void getCollectionReferencedEntity_(Object objectValue, Class entityClass, int depth) {
        ...
        for (Object entityReference : entityReferences) {
            if (entityReference == null || (entityReference instanceof AbstractEntityRef) == false) {
                continue;
            }

            getReferencedEntity_((AbstractEntityRef) entityReference, entityClass, depth);
        }
    }

    private void getReferencedEntity_(AbstractEntityRef entityReference, Class entityClass, int depth) {
        try {
