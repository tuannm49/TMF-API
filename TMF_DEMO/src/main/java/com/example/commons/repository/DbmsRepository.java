 ...
 */
public class DbmsRepository<T> implements GenericRepository<T>{
     ...
     * @param entityClass
     */
    public DbmsRepository(EntityManager entityManager, Class<T> entityClass) {
...
//    public List<T> findByCriteria(MultivaluedMap<String, String> map, Class<T> clazz) {
    public List<T> findByCriteria(Map<String, List<String>> map, Class<T> clazz) {
        ...
        try {
            ...
            Logger.getLogger(DbmsRepository.class.getName()).log(Level.INFO, "findByCriteria error, null result returned", ex);
            return null;
        }
    }
