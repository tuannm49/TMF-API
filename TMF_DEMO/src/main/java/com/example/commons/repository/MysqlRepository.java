...
import com.example.commons.exceptions.ExceptionType;
import com.example.commons.exceptions.UnknownResourceException;
import com.example.commons.utils.TMFDate;
 ...
 */
public class MysqlRepository<T> implements GenericRepository<T>{
     ...
     * @param entityClass
     */
    public MysqlRepository(EntityManager entityManager,Class<T> entityClass) {
...
//    public List<T> findByCriteria(MultivaluedMap<String, String> map, Class<T> clazz) {
    public List<T> findByCriteria(Map<String, List<String>> map, Class<T> clazz) {
        ...
        try {
            ...
            Logger.getLogger(MysqlRepository.class.getName()).log(Level.INFO, "findByCriteria error, null result returned", ex);
            return null;
        }
    }
