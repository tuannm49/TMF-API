...
// Interface cho Action Delegation
interface ActionDelegate {
    String execute(Task task);
}

// Interface cho Subtask Delegation
interface SubtaskDelegate {
    List<Task> getSubtasks(Task parent);
}

// Lớp Task sử dụng ActionDelegate và SubtaskDelegate
class Task {
    private final String taskId;
    private final ActionDelegate actionDelegate;
    private final SubtaskDelegate subtaskDelegate;
    ...
    public Task(String taskId, ActionDelegate actionDelegate, SubtaskDelegate subtaskDelegate, int priority, int maxRetries) {
        this.taskId = taskId;
        this.actionDelegate = actionDelegate != null ? actionDelegate : task -> "Default result for " + task.getTaskId();
        this.subtaskDelegate = subtaskDelegate != null ? subtaskDelegate : parent -> new ArrayList<>();
        this.status = "PENDING";
    ...
    public List<Task> getSubtasks() { return subtaskDelegate.getSubtasks(this); }
    public int getPriority() { return priority; }
    public int getMaxRetries() { return maxRetries; }
    public int getRetryCount() { return retryCount; }
    public void incrementRetryCount() { this.retryCount++; }
    public String executeAction() { return actionDelegate.execute(this); }
}
    ...
    private void processTask(Task task) {
        ...
        try {
            ...
            CompletableFuture<String> actionFuture = CompletableFuture.supplyAsync(
                    task::executeAction, executor); // Gọi ActionDelegate
...
// Lớp sinh cây task với ActionDelegate và SubtaskDelegate
class TaskTreeGenerator {
    ...
    public static Task generateTaskTree(int depth, int maxSubtasks, String parentId) {
        ...
        ActionDelegate actionDelegate = task -> sampleAction(task.getTaskId());
        SubtaskDelegate subtaskDelegate = parent -> {
            List<Task> subtasks = new ArrayList<>();
            int numSubtasks = random.nextInt(maxSubtasks + 1);
            for (int i = 0; i < numSubtasks && depth > 1; i++) {
                Task subtask = generateTaskTree(depth - 1, maxSubtasks, parent.getTaskId());
                if (subtask != null) subtasks.add(subtask);
            }
            return subtasks;
        };

        return new Task(taskId, actionDelegate, subtaskDelegate, priority, maxRetries);
    }
